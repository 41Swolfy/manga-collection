...
function sum(){
    ...
    Promise.all(urls.map(url => fetch(url).then(response => response.text())))
      .then(htmls => {
        ...
        document.getElementById("manga_title").innerHTML ='Anzahl an gesammelten Manga: '+sum+'  | '+ 'Geschätzte Ausgaben : ' + (8.385583524* sum).toFixed(2)+'€' ; // 8.385583524 is the fixed avg spent per manga      
